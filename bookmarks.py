...
class bmgo(_bm):
    ...
    param_def = [
        ["name", Type.String, None, "Name of the bookmark to go to"],
        [
            "parallel",
            Type.Boolean,
            False,
        ...
        [
            ...
            Optional,
            "Motors to exclude from movement",
        ],
    ]
    interactive = True
    ...
    def run(self, fname):
        ...
        if fname is None:
            if self.autosavefile is None:
                self.warning("No filename given and backup file not set. " "Aborting.")
                return
...
class bm_import(_bm):
    ...
    param_def = [["fname", Type.String, None, "json file name"]]
